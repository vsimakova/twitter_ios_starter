...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    static let client = TwitterAPICaller(baseURL: URL(string: "https://api.twitter.com"), consumerKey: "JRbI3Qc4MyqwNDIfgpbgxMg14", consumerSecret: "f6fCrZGBKehsc8CMu62IEP4d04nmHyyOWdmm1fhH3NpjQulmsn")
    var loginSuccess: (() -> ())?
    ...
    func favoriteTweet(tweeetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        ...
        TwitterAPICaller.client?.post(url, parameters: ["id":tweeetId], progress: nil, success: {
    ...
    func retweet (tweeetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let url = "https://api.twitter.com/1.1/statuses/retweet/\(tweeetId).json"
        TwitterAPICaller.client?.post(url, parameters: ["id":tweeetId], progress: nil, success: {
            (task: URLSessionDataTask, response: Any?) in success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
